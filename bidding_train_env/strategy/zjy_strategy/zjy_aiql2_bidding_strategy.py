...
from bidding_train_env.zjy_train.attention_iql2.aiql import AIQL,Q,V,Actor,TimeEncoding,Attention  # 确保这个导入路径是正确的
...
class IqlBiddingStrategy(BaseBiddingStrategy):
    ...
    def bidding(self, timeStepIndex, pValues, pValueSigmas, historyPValueInfo, historyBid,
        ...
        historical_CPA = np.array([np.sum(np.multiply(history_cost_i, history_exposure_i))/(1e-5+np.sum(history_conversion_i)) 
                                   for history_cost_i, history_exposure_i,history_conversion_i in zip(history_cost, history_exposure, history_conversion)]) if history_cost else np.zeros(1)
        ...
        historical_cost = np.array([np.sum(np.multiply(cost, is_exposure)) for cost,is_exposure in zip(history_cost, history_exposure)]) if history_cost else np.zeros(1)

        historcial_actions = np.array(self.historical_actions) if self.historical_actions else np.zeros(1)

        history_informations = torch.as_tensor(np.nan_to_num(np.array([ 
        ...
        current_pValues_quantile1 = np.percentile(pValues,75,interpolation='linear')
        current_pValues_quantile2 = np.percentile(pValues,25,interpolation='linear')
        current_pValues_std = np.std(pValues)
        ...
        current_informations = torch.tensor([
            ...
            int(self.category ==3),
            int(self.category ==4),
            int(self.remaining_budget < 0.1),
            int(timeStepIndex == 47),
        ]).reshape(1, 1, -1).type(torch.float32) # 这样就变成了 [1, 1, D] 维度的向量
        
        self.history_budgets.append(budget_left)

        alpha = self.model(current_informations, history_informations).view(-1)
        alpha = alpha.cpu().numpy() * 50 +80
        bids = alpha * pValues

        self.historical_actions.append(alpha.item())
        return bids
