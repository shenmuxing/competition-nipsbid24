...
class OnlineLp:
    ...
    def __init__(self, dataPath, **kwargs):
        ...
        self.config = kwargs.get('config', {"IS_DOCKER":False})
    ...
    def train(self, save_path):
        ...
        if is_csv:
            ...
            feather_files = glob.glob(os.path.join(self.dataPath, '*.feather'))
            print(feather_files)
            feather_files = sorted(feather_files)
            # select episode-0
            for i, feather_file_path in enumerate(feather_files[0:1]):
                print("开始分析" + feather_file_path)
                df = pd.read_feather(feather_file_path)
                episodeRes = self.onlinelp_for_specific_episode(df)
                episodeRes.to_csv(f'{save_path}/period.csv', index=False)
                print("完成分析" + feather_file_path)
