...
Experience = namedtuple("Experience", field_names=["state_current_informations","state_history_informations", "action", "reward", 
                                                   "next_state_current_informations","next_state_history_informations", "done",
                                                    "state_current_informations_other","next_state_current_informations_other"])
...
class ReplayBuffer:
    ...
    def push(self, state_current_informations, state_history_informations, 
             ...
             state_current_informations_other,next_state_current_informations_other,device = 'cpu'):
        """saving an experience tuple"""
        experience = Experience(torch.tensor(state_current_informations).to(device),torch.tensor(state_history_informations).to(device), torch.tensor(action).to(device), 
                                torch.tensor(reward).to(device), torch.tensor(next_state_current_informations).to(device), torch.tensor(next_state_history_informations).to(device),
                                torch.tensor(done).to(device),torch.tensor(state_current_informations_other).to(device),torch.tensor(next_state_current_informations_other).to(device)
                                )
