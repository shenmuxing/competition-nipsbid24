...
def train_iql_model():
    ...
    if config["IS_DOCKER"]:
        ...
        train_data_path = "./data/traffic/training_data_rlData_folder/training_data_all-rlData.feather"
        training_data = pd.read_feather(train_data_path)
    ...
    def safe_literal_eval(val):
        ...
        try:
            ...
            print(ValueError)
            return val
    if config["IS_DOCKER"]: # 这里是处理csv文件中输入是一个个文本的问题的，feather直接保存为np.ndarray，不会出这种问题，故直接跳过
